...
// The deallocation function is lean, but it does a lot of assertions (which still don't catch all error conditions).
// Chunk respects the grand C and C++ tradition in memory allocation: Prepare for the worst if you pass the
// wrong pointer to Chunk::Deallocate .
